...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  //主模板
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.admin}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    ...
    ,'<div class="layui-table-page">'
      ,'<div id="layui-table-page{{d.admin}}"></div>'
    ,'</div>'
    ,'{{# } }}'
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      layui.each(data, function(i1, item1){
        ...
        trs.push('<tr data-admin="'+ i1 +'">'+ tds.join('') + '</tr>');
        trs_fixed.push('<tr data-admin="'+ i1 +'">'+ tds_fixed.join('') + '</tr>');
        trs_fixed_r.push('<tr data-admin="'+ i1 +'">'+ tds_fixed_r.join('') + '</tr>');
      });
  ...
  Class.prototype.setThisRowChecked = function(index){
    ...
    ,tr = that.layBody.find('tr[data-admin="'+ index +'"]');
